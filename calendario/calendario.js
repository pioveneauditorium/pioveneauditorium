...
fetch('https://docs.google.com/spreadsheets/d/e/2PACX-1vRSqROrdJEDeejhnLMrFq9tTIvX4XUTRz8719e9xflNmyNAYaQB3h_JfM8E9Mes5AVKgaXGKMIDo-pN/pub?output=csv')
  ...
  .then(data => {
    ...
    // Header: Data,Orario,Titolo,Descrizione,Immagine,linkBiglietti,Tipo
    rows.forEach((row, index) => {
      ...
      if (columns.length >= 7) {
        ...
        const tipo = columns[6].trim(); // Nuova colonna Tipo

        events[date] = { time, title, description, image, linkBiglietti, tipo };
      }
    });
    renderCalendar();
  })
  ...
  for (let day = 1; day <= numDays; day++) {
    ...
    if (events[dateStr]) {
      switch(events[dateStr].tipo) {
        ...
        default: eventClass = 'highlighted'; // colore di default
      }
    }
